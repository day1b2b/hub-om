...
## 구조 선행 기준
S1 모든 단계 Core/Shell/Check 태그, S2 Core 1개 이상, S3 입력 필드와 분기/변환 규칙 명시. 하나라도 FAIL이면 결과 기준 평가 중단.
V6/V9/V14 추가: domain별 모델 집합을 명시해 준비하고 검사한다. Operation 기존 모델/카운터 계약은 변경하지 않는다. 권한 판단은 기존 호출부 책임이며 실제 운영 권한 검증 완료를 주장하지 않는다.

추가 검증 해석: V8은 공개 합성 fixture 소스 자체를 금지하지 않는다. 실제 저장문서/런타임 로그의 원문 PII 누출을 판단한다. Mongo7 보조 엔진 결과는8.0 운영 대상의 엔진검증을 대체하지 않는다.
