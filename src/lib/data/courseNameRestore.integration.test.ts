...
test("과정명 복원: 격리 PostgreSQL의 데이터·경합·감사 원자성", { skip: !url }, async (t) => {
  ...
  async function fixture() {
    ...
    const company = await db.company.create({ data: { name: key, normalizedName: key } });
    companyIds.push(company.id);
    const course = await db.course.create({ data: { companyId: company.id, courseId: key, name: "현재 과정",
      operationType: "LONG", courseCategory: "category", tools: "tool", revenue: 1234, revenueRaw: "1,234" } });
    const run = await db.dataImportRun.create({ data: { sourceType: "test", sourceName: key } });
    runIds.push(run.id);
    const session = await db.operationSession.create({ data: { operationId: key, courseRecordId: course.id,
      startDate: date, endDate: date, lectureManagementNote: "preserve-me" } });
    const source = await db.operationSourceRecord.create({ data: { importRunId: run.id, operationSessionId: session.id,
  ...
  try {
    await t.test("새 과정 메타데이터와 선택하지 않은 회차·기록을 보존하고 중복 재시도는 거절한다", async () => {
      ...
      const untouched = await db.operationSession.create({ data: { operationId: randomUUID(), courseRecordId: f.course.id, startDate: date, endDate: date } });
      ...
      const changed = await db.operationSession.findUniqueOrThrow({ where: { id: f.session.id }, include: { course: true } });
      ...
      assert.equal((await db.operationSession.findUniqueOrThrow({ where: { id: untouched.id } })).courseRecordId, f.course.id);
      assert.equal(await db.activityChange.count({ where: { requestId: call.requestId } }), 2);
    ...
    await t.test("동일 이름의 다른 코스는 제외하고 제로폭 공백이 있는 기존 대상을 재사용한다", async () => {
      const f = await fixture();
      await db.course.create({ data: { companyId: f.company.id, courseId: "other", name: "원래 과정" } });
      const target = await db.course.create({ data: { companyId: f.company.id, courseId: `${f.key}\u200b`, name: "원래 과정", tools: "existing" } });
      const plan = await planCourseNameRestore(f.key, db);
      await applyCourseNameRestore(f.key, [f.key], plan.snapshot, null, db);
      assert.equal((await db.operationSession.findUniqueOrThrow({ where: { id: f.session.id } })).courseRecordId, target.id);
      assert.equal((await db.course.findUniqueOrThrow({ where: { id: target.id } })).tools, "existing");
    });
    await t.test("원천 수정·대상 수정·소프트 삭제 후 오래된 계획은 쓰기 전에 거절한다", async () => {
      for (const change of ["source", "course", "deleted"]) {
        ...
        if (change === "source") await db.operationSourceRecord.update({ where: { id: f.source.id }, data: { mappedFields: { courseName: "바뀐 원천" } } });
        if (change === "course") await db.course.update({ where: { id: f.course.id }, data: { tools: "changed" } });
        if (change === "deleted") await db.operationSession.update({ where: { id: f.session.id }, data: { deletedAt: new Date() } });
        await assert.rejects(applyCourseNameRestore(f.key, [f.key], plan.snapshot, null, db), CourseNameRestoreConflict);
        assert.equal(await db.course.count({ where: { companyId: f.company.id } }), 1);
      }
    });
    await t.test("정규화 중복 대상과 원천 최신 시각 동률은 임의로 고르지 않는다", async () => {
      const f = await fixture();
      for (const id of [f.key, `${f.key}\u200b`]) await db.course.create({ data: { companyId: f.company.id, courseId: id, name: "원래 과정" } });
      assert.equal((await planCourseNameRestore(f.key, db)).rows[0].restorable, false);
      const g = await fixture();
      await db.operationSourceRecord.create({ data: { importRunId: g.run.id, operationSessionId: g.session.id, sourceWorkbook: "test",
    ...
    await t.test("같은 새 대상의 메타데이터가 다르면 복원을 차단한다", async () => {
      const f = await fixture();
      const course = await db.course.create({ data: { companyId: f.company.id, courseId: f.key, name: "현재 과정2", tools: "different" } });
      const session = await db.operationSession.create({ data: { operationId: randomUUID(), courseRecordId: course.id, startDate: date, endDate: date } });
      await db.operationSourceRecord.create({ data: { importRunId: f.run.id, operationSessionId: session.id, sourceWorkbook: "test",
    ...
    await t.test("두 번째 회차 실패 시 첫 회차·새 과정·감사 이력까지 전부 롤백한다", async () => {
      const f = await fixture();
      const second = await db.operationSession.create({ data: { operationId: "restore-test-fail", courseRecordId: f.course.id, startDate: date, endDate: date } });
      await db.operationSourceRecord.create({ data: { importRunId: f.run.id, operationSessionId: second.id, sourceWorkbook: "test",
        sourceSheet: "test", sourceRowNumber: 2, rowSnapshot: {}, mappedFields: { courseName: "원래 과정" } } });
      ...
      try {
        ...
        assert.equal(await db.course.count({ where: { companyId: f.company.id } }), 1);
        assert.equal((await db.operationSession.findUniqueOrThrow({ where: { id: f.session.id } })).courseRecordId, f.course.id);
        assert.equal(await db.activityChange.count({ where: { requestId: call.requestId } }), 0);
      } finally {
    ...
    await t.test("동시 적용 두 건 중 한 건만 성공하고 과정은 한 번만 생성한다", async () => {
      ...
      assert.equal(await db.course.count({ where: { companyId: f.company.id, name: "원래 과정" } }), 1);
    });
    ...
    await t.test("같은 코스ID의 기업을 행마다 표시하고 선택한 기업만 변경한다", async () => {
      ...
      await db.course.update({ where: { id: g.course.id }, data: { courseId: f.key } });
      const plan = await planCourseNameRestore(f.key, db);
      assert.equal(new Set(plan.rows.map((row) => row.companyName)).size, 2);
      await applyCourseNameRestore(f.key, [f.key], plan.snapshot, null, db);
      assert.equal((await db.operationSession.findUniqueOrThrow({ where: { id: g.session.id } })).courseRecordId, g.course.id);
    });
  } finally {
    await db.operationSourceRecord.deleteMany({ where: { importRunId: { in: runIds } } });
    await db.operationSession.deleteMany({ where: { course: { companyId: { in: companyIds } } } });
    await db.course.deleteMany({ where: { companyId: { in: companyIds } } });
    await db.company.deleteMany({ where: { id: { in: companyIds } } });
    await db.dataImportRun.deleteMany({ where: { id: { in: runIds } } });
