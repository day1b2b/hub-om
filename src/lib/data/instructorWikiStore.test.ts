...
test("다른 노트를 가리키는 notionId만 수동 연결로 본다", () => {
  const targets = resolveNotionLinkTargets([
    ...
    { instructorName: "김진태", notionId: ID_A, notion: { syncedAt: "2026-08-20T00:00:00.000Z" } },
    // OM이 연결한 운영 현황 표기
    { instructorName: "디노랩스_김진태", notionId: ID_A },
    // 연결 안 된 표기
    { instructorName: "강완주" }
  ]);
...
test("본체 자신은 연결 대상에서 빠진다", () => {
  const targets = resolveNotionLinkTargets([
    { instructorName: "김진태", notionId: ID_A, notion: { syncedAt: "x" } },
  ]);
...
test("가리키는 대상이 없는 notionId는 무시한다", () => {
  const targets = resolveNotionLinkTargets([
    { instructorName: "김진태", notionId: ID_A, notion: { syncedAt: "x" } },
    { instructorName: "떠돌이표기", notionId: ID_B },
  ]);
  assert.deepEqual(targets, {});
});
...
test("같은 노션 ID를 두 노트가 가지면 syncedAt이 최신인 쪽을 정본으로 본다", () => {
  ...
  const targets = resolveNotionLinkTargets([
    { instructorName: "디노랩스_김진태", notionId: ID_A, notion: { syncedAt: "2026-07-01T00:00:00.000Z" } },
    { instructorName: "김진태", notionId: ID_A, notion: { syncedAt: "2026-08-20T00:00:00.000Z" } },
  ]);
  assert.deepEqual(targets, { "디노랩스_김진태": "김진태" });
});
...
test("syncedAt이 같으면 이름이 짧은 쪽을 정본으로 본다(결정적)", () => {
  ...
  const targets = resolveNotionLinkTargets([
    { instructorName: "디노랩스_김진태", notionId: ID_A, notion: { syncedAt: same } },
    { instructorName: "김진태", notionId: ID_A, notion: { syncedAt: same } },
  ]);
  assert.deepEqual(targets, { "디노랩스_김진태": "김진태" });
