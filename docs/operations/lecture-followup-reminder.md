...
- **D+1(종료 다음 날)**: 코스ID·강의관리 시트·만족도 중 아직 등록되지 않은 항목 안내.
...
| 항목 | 판정 기준 | 화면 위치 |
| --- | --- | --- |
| 코스ID | 회차의 `courseId` 값이 비어 있지 않으면 등록됨 | 운영 상세 > 회차 정보 |
...
- 등록/작성 판정은 운영현황이 쓰는 아카이빙 완료 조건(`operationCalculations.isArchiveComplete`)과 같은 "값이 비어 있으면 미등록" 규칙을 쓴다. 결과보고서는 필요 여부(`hasResultReport`)가 회차마다 달라 이 알림에서는 보지 않는다.
- 대상 회차는 운영현황 화면과 같은 리포지토리(`getOperationRepository().listOperations()`)에서 읽는다. 화면에 보이는 값과 알림 판정이 어긋나지 않는다.
