...
운영 상세페이지의 메일 논의 영역은 **수동 아카이브 파일만** 읽습니다.
Gmail API 연동은 2026-09-01에 제거했습니다 — Google 인증 심사에서 `gmail.readonly`가
민감 범위로 분류돼 심사 난도를 크게 올리는데, 실사용 가치가 그만큼은 아니라고 판단했습니다.
로그인 시 요청하는 Google 범위에도 Gmail은 더 이상 포함되지 않습니다.

- `GMAIL_DISCUSSION_MANUAL_ARCHIVE_FILE`: 수동으로 정리한 과거 그룹 메일 아카이브 JSON 파일입니다. `.local/`처럼 gitignore된 경로에 둡니다. 이 값이 없으면 상세페이지의 메일 버튼은 `메일 설정 필요`로 표시됩니다.
- `GMAIL_DISCUSSION_MANUAL_ARCHIVE_UNTIL_DATE`: 수동 아카이브가 커버하는 마지막 날짜입니다.

환경변수 이름의 `GMAIL_` 접두사는 예전 이름을 그대로 둔 것이고, 지금은 Gmail API를 부르지 않습니다.
