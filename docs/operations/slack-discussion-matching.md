...
요약은 `요지`, `결론`, `후속`, `맥락` 같은 라벨을 붙이지 않습니다.
또한 `교안 공유`, `신경식 강사`, `피드백 반영`처럼 명사형 키워드만 나열하지 않습니다.
각 bullet은 운영자가 읽고 판단할 수 있는 문장형 요약이어야 합니다.
좋은 요약은 "무엇이 논의됐는지", "누가 무엇을 요청/공유/확정했는지", "다음 확인이 필요한지" 중 하나 이상을 포함합니다.

예시:

```text
- 신경식 강사에게 전달할 교안과 강의 내용을 공유한 스레드입니다.
- 수강생 피드백을 강사에게 전달해 다음 회차에 반영하도록 정리한 스레드입니다.
- 강사비 지급 기준을 4시간에서 2시간으로 조정할 수 있는지 논의한 스레드입니다.
```
...
AI 요약이 꺼져 있거나 실패하면 규칙 기반 fallback 요약을 사용하지만, fallback도 명사형 키워드가 아니라 문장형 운영 요약을 출력해야 합니다.
