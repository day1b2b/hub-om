...
이 문서는 운영 상세페이지의 `Slack 논의` 영역에 어떤 Slack 스레드를 보여줄지 정리한 운영 기준입니다.
관리자/매니저가 기준을 조정할 때 이 문서를 먼저 확인합니다.

## 목적

과정 운영에서 중요한 논의는 Slack 스레드 안에서 많이 발생합니다.
상세페이지의 Slack 논의 영역은 과정과 관련된 운영논의 스레드를 빠르게 열어볼 수 있게 하는 것이 목적입니다.
...
운영보고 채널은 `Slack 논의` 영역이 아니라 `자료 링크`의 `강의보고` 링크로 표시합니다.
